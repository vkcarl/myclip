...
EditRate& GraphicsWidgetTimeline::TimelinePainter::GetCplEditRate() const {
    return FrameRateManager::getEditRate();
}

GraphicsWidgetTimeline::TimelinePainter::TimelinePainter(GraphicsWidgetTimeline *parent):
    GraphicsWidgetBase(parent)
{
    FrameRateManager::max_edit_rate_in_all_file_ = EditRate::EditRate30;
...
{
    const qreal lod = option->levelOfDetailFromTransform(painter->worldTransform());    //这应该就是记录用鼠标滚轴放大的程度的变量
